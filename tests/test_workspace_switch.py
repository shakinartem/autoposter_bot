...
def test_same_session_token_switches_workspace_only_with_membership(tmp_path):
    db_path = tmp_path / "switch.sqlite3"
    ...
    with raw.connect() as db:
        ...
        db.execute(
            "INSERT INTO workspaces(id, name, owner_user_id, created_at) VALUES (6, 'Beta', 1, ?)",
            (now,),
        )
        db.execute(
            "INSERT INTO workspaces(id, name, owner_user_id, created_at) VALUES (7, 'Forbidden', 1, ?)",
            (now,),
        )
