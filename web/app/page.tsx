...
export default function HomePage() {
  ...
  const loadVariants = useCallback(async (item: ContentItem) => {
    ...
    const byPlatform = Object.fromEntries(rows.map((variant) => [variant.platform, variant]));
    setVariants(byPlatform);
  }, []);
  ...
  const bootstrap = useCallback(async () => {
    try {
      const [platformData, accountData, contentData] = await Promise.all([
        listPlatforms(),
        listAccounts(),
        listContent(),
      ]);
  ...
  }, [activePlatform, accounts, masterBody, masterTitle, variants]);
  ...
  async function saveMaster() {
    ...
    try {
      ...
      if (activeContent) {
        saved = await updateContent(activeContent.id, {
          title: masterTitle,
          body: masterBody,
        });
  ...
  async function savePlatformVariant() {
    ...
    try {
      ...
      setNotice(`${label(activePlatform)} сохранён как отдельная версия`);
      return variant;
    } catch (error) {
  ...
  async function handlePublish(mode: "now" | "schedule" | "dry") {
    ...
    try {
      ...
      if (result.ok) {
        setNotice(mode === "dry" ? "Dry-run прошёл успешно" : `${label(activePlatform)} опубликован`);
      } else {
        setNotice(result.error_message || "Площадка вернула ошибку публикации");
      }
    } catch (error) {
    ...
    <div className="shell">
      <aside className="sidebar">
        <div className="brand">
          ...
          <div>
            <strong>Autoposter</strong>
            <span>Content Distribution OS</span>
          </div>
        </div>
        ...
        <nav className="nav">
          ...
          <button className="navItem">Календарь</button>
          <button className="navItem">Публикации</button>
          <button className="navItem">Аналитика</button>
          <button className="navItem">Аккаунты <span>{accounts.length}</span></button>
        </nav>
        ...
        <div className="library">
          ...
          {content.slice(0, 7).map((item) => (
            <button
              key={item.id}
              className={`libraryItem ${activeContent?.id === item.id ? "selected" : ""}`}
              onClick={() => void selectContent(item)}
            >
              <span className="libraryDot" />
              <span>{item.title || firstLine(item.body) || "Без названия"}</span>
            </button>
          ))}
          {!content.length && <p className="emptySmall">Здесь появятся ваши материалы.</p>}
        </div>

        <div className="sidebarFooter">
          <span className={`statusDot ${apiOnline ? "online" : ""}`} />
          <div>
            <strong>{apiOnline ? "Backend online" : "Backend offline"}</strong>
            <span>{API_URL}</span>
          </div>
        </div>
      </aside>
      ...
      <main className="main">
        <header className="topbar">
          <div>
            <span className="eyebrow">CONTENT / COMPOSER</span>
            <h1>{activeContent ? masterTitle || "Без названия" : "Новый материал"}</h1>
          </div>
          <div className="topActions">
            <span className={`connectionBadge ${apiOnline ? "ok" : "error"}`}>{notice}</span>
            <button className="ghostButton" onClick={() => void handlePublish("dry")} disabled={busy || !activeContent}>
              Проверить
            </button>
            <button className="primaryButton" onClick={() => void handlePublish("now")} disabled={busy || !activeContent}>
              Опубликовать
            </button>
          </div>
        </header>
          ...
          <section className="editorPanel">
            <div className="masterCard">
              <div className="cardHeader">
                <div>
                  <span className="eyebrow">MASTER CONTENT</span>
                  <h2>Одна идея — разные площадки</h2>
                </div>
                <button className="saveButton" onClick={() => void saveMaster()} disabled={busy || !apiOnline}>
                  {busy ? "Сохраняю…" : "Сохранить master"}
                </button>
              </div>
              <input
              ...
              />
              <textarea
                className="masterTextarea"
                value={masterBody}
                onChange={(event) => setMasterBody(event.target.value)}
                placeholder="Напишите исходный материал. Это источник, из которого будут собираться версии для каждой площадки…"
              />
              <div className="masterFooter">
                <span>{masterBody.length} символов</span>
                <span>AI adaptation — следующий слой</span>
              </div>
            </div>
            ...
            <div className="platformSection">
              <div className="platformTabs" role="tablist">
                {Object.keys(platforms).map((platform) => (
                  <button
                    key={platform}
                    className={`platformTab ${activePlatform === platform ? "active" : ""}`}
                    onClick={() => setActivePlatform(platform)}
                  >
                    <span className={`platformIcon ${platform}`}>{label(platform).slice(0, 2)}</span>
                    {label(platform)}
                    {variants[platform] && <span className={`syncMini ${variants[platform].sync_with_master ? "synced" : "locked"}`} />}
                  </button>
                ))}
              </div>
              ...
              <div className="variantCard">
                <div className="variantHeader">
                  <div>
                    <span className="eyebrow">PLATFORM VARIANT</span>
                    <h2>{label(activePlatform)}</h2>
                  </div>
                ...
                <div className="capabilities">
                  {(capability?.content_types ?? []).slice(0, 5).map((type) => <span key={type}>{type}</span>)}
                </div>
                ...
                <input
                  ...
                  placeholder={`${label(activePlatform)} title`}
                />
                ...
                <textarea
                  ...
                  placeholder={`Версия для ${label(activePlatform)}`}
                />

                <div className="variantFooter">
                  <div className="metaRow">
                    <span>{variantText.length} символов</span>
                    <span>revision {activeVariant?.revision ?? 1}</span>
                  </div>
                  <button className="secondaryButton" onClick={() => void savePlatformVariant()} disabled={!activeContent || busy}>
                    Сохранить версию
                  </button>
                </div>
              </div>
          ...
          <aside className="publishPanel">
            <div className="publishCard">
              ...
              {!platformAccounts.length && (
                <p className="hint warning">Для {label(activePlatform)} пока нет подключённого аккаунта. Старые подключения из Telegram-бота автоматически появятся здесь.</p>
              )}
              ...
              <button className="primaryButton full" onClick={() => void handlePublish("schedule")} disabled={busy || !activeContent || !selectedAccount}>
                Поставить в календарь
              </button>
              <button className="ghostButton full" onClick={() => void handlePublish("now")} disabled={busy || !activeContent || !selectedAccount}>
                Опубликовать сейчас
              </button>
            </div>
            ...
            <div className="insightCard">
              ...
              <div className="capabilityList">
                {Object.entries(capability?.features ?? {}).map(([feature, enabled]) => (
                  <div key={feature}>
                    <span>{humanize(feature)}</span>
                    <strong>{enabled ? "Да" : "Нет"}</strong>
                  </div>
                ))}
                {!Object.keys(capability?.features ?? {}).length && <p className="hint">Adapter пока не описал дополнительные capabilities.</p>}
              </div>
            </div>

            <div className="architectureNote">
              <strong>Content → Variant → Publication</strong>
              <p>Каждая площадка публикуется и ретраится независимо. Ошибка Instagram больше не ломает успешный Telegram-пост.</p>
            </div>
          </aside>
        </div>
      </main>
