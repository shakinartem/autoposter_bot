...
class ContentFactoryLedger:
    """Backend-aware durable state for Content Factory integration.

    The Content OS store owns content/variants/publications. This ledger only owns
    cross-application identity, idempotency and feedback delivery state.
    """
    ...
    def init_schema(self) -> None:
        ...
        files = ["005_postgres_content_factory_bridge.sql"] if self.backend == "postgres" else [
        ...
        *,
        ...
        with self.root.connect() as connection:
            existing = connection.execute(
                self._sql("SELECT id FROM content_factory_analytics_snapshots WHERE event_id = ?"),
                (event_id,),
            ).fetchone()
            if existing:
                outbox = connection.execute(
                    self._sql("SELECT status FROM content_factory_feedback_outbox WHERE analytics_snapshot_id = ?"),
                    (existing["id"],),
                ).fetchone()
                return {
                    "status": "duplicate",
                    ...
                    "feedback_status": outbox["status"] if outbox else "not_linked",
                }

            if self.backend == "postgres":
                snapshot = connection.execute(
                    """INSERT INTO content_factory_analytics_snapshots(
                        publication_id, event_id, platform, metrics_json, metadata_json, captured_at, created_at
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id""",
                    (publication_id, event_id, platform, Jsonb(metrics), Jsonb(metadata), captured_at, now_dt),
                ).fetchone()
                ...
                cursor = connection.execute(
                    ...
                    (publication_id, event_id, platform, json.dumps(metrics), json.dumps(metadata), captured_at.isoformat(), now),
                )
            ...
            connection.execute(
                self._sql(
                    """INSERT INTO content_factory_feedback_outbox(
                        id, analytics_snapshot_id, event_id, source_content_id, external_publication_id,
                        payload_json, status, attempts, available_at, created_at, updated_at
                    ) VALUES (?, ?, ?, ?, ?, ?, 'pending', 0, ?, ?, ?)"""
                ),
                (feedback_id, snapshot_id, event_id, source_content_id, external_publication_id, self._json(payload), now, now, now),
            )
    ...
    def mark_feedback_failed(self, feedback_id: str, *, attempts: int, error: str) -> None:
        now_dt = datetime.now(timezone.utc)
        delay = min(3600, max(5, 2 ** min(attempts, 10)))
        available = now_dt + timedelta(seconds=delay)
        with self.root.connect() as connection:
            connection.execute(
