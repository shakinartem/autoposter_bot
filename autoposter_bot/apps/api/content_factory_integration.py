...
class ContentFactoryIntegration:
    def __init__(self, *, persistence: Any, media_storage: Any, registry: Any) -> None:
        ...
        raw_workspace = os.getenv("CONTENT_FACTORY_DEFAULT_WORKSPACE_ID", "").strip()
        default_workspace_id = int(raw_workspace) if raw_workspace else None
    ...
    def _authorize_package(self, authorization: str | None) -> None:
        if not self.ingest_token:
            raise HTTPException(503, "Content Factory ingestion is disabled")
    ...
    def _authorize_analytics(self, token: str | None) -> None:
        if not self.analytics_token:
            raise HTTPException(503, "Analytics ingestion is disabled")
    ...
    def _mount(self) -> None:
        ...
        def ingest_package(package: ContentPackageV1, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"), authorization: str | None = Header(default=None, alias="Authorization")):
            self._authorize_package(authorization)
            if not idempotency_key or len(idempotency_key) > 255:
                raise HTTPException(400, "A valid Idempotency-Key is required")
            try:
                return self.bridge.ingest(package, idempotency_key=idempotency_key, supported_platforms=set(self.registry.capabilities()))
        ...
        def record_performance(publication_id: str, payload: PerformanceSnapshotRequest, analytics_token: str | None = Header(default=None, alias="X-Analytics-Token")):
            self._authorize_analytics(analytics_token)
            try:
                result = self.feedback.record_snapshot(publication_id, metrics=payload.metrics, captured_at=payload.captured_at, event_id=payload.event_id, metadata=payload.metadata)
        ...
        def dispatch_performance(analytics_token: str | None = Header(default=None, alias="X-Analytics-Token")):
            self._authorize_analytics(analytics_token)
