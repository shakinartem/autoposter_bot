...
class ContentFactoryBridge:
    ...
    def ingest(self, package: ContentPackageV1, *, idempotency_key: str, supported_platforms: set[str]) -> dict[str, Any]:
        with self.ledger.delivery_lock(idempotency_key):
            return self._ingest_locked(package, idempotency_key=idempotency_key, supported_platforms=supported_platforms)

    def _ingest_locked(self, package: ContentPackageV1, *, idempotency_key: str, supported_platforms: set[str]) -> dict[str, Any]:
        ...
        if existing:
            ...
            response = dict(existing.get("response_json") or {})
        ...
        content_application = ContentApplication(scoped)
        ...
        for variant in package.variants:
            for ref in variant.media:
                ...
                if asset.id not in seen_media:
                    master_media.append(asset); seen_media.add(asset.id)
            for tag in variant.hashtags:
                if tag not in seen_hashtags:
                    hashtags.append(tag); seen_hashtags.add(tag)
        ...
        item.metadata = {
            **(item.metadata or {}),
            "content_factory": {
                ...
                "lineage": package.lineage.model_dump(mode="json") if package.lineage else None,
            },
        }
        ...
        for payload in package.variants:
            ...
            variant = content_application.upsert_variant(
                local_content_id,
                platform,
            ...
            if variant is None:
                raise RuntimeError(f"Unable to persist platform variant: {platform}")
